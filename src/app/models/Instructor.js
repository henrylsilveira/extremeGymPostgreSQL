...
module.exports = {
    ...
    update(data, callback) {
        const query = `UPDATE instructors SET avatar_url=($1), name=($2), birth=($3), gender=($4), services=($5) WHERE id = $6`
    ...
    paginate(params){
        ...
        if(filter){
            filterQuery = `${query} WHERE instructors.name ILIKE '%${filter}%' OR instructors.services ILIKE '%${filter}%'`;
        ...
        FROM instructors 
        LEFT JOIN members 
        ON (instructors.id = members.instructor_id) ${filterQuery}
